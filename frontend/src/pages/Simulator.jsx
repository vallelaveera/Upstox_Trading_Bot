...
import { Lightning, ArrowsClockwise, ChartLineUp } from "@phosphor-icons/react";
...
export default function Simulator() {
  ...
  const runSim = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.post(`${API}/simulate`, payload, { timeout: 90000 });
      setResult(res.data);
      toast.success(
        `Simulation complete · ${res.data.kpis.total_trades} trades · ${res.data.kpis.win_rate}% win rate`
      );
    } catch (e) {
      console.error(e);
      ...
      <header className="border-b border-white/5 sticky top-0 z-30 backdrop-blur-md bg-[#050505]/80">
        <div className="max-w-[1600px] mx-auto px-6 md:px-8 py-5 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-display text-xl md:text-2xl font-bold tracking-tight leading-none" data-testid="app-title">
                NSE SWING <span className="text-[#E2FF00]">SIM</span>
              </h1>
              <p className="text-[11px] text-neutral-500 uppercase tracking-[0.2em] mt-1">
                Buy the dip · Sell the rip · Backtest
              </p>
            </div>
          </div>
      ...
      <main className="max-w-[1600px] mx-auto p-4 md:p-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 md:gap-6">
          ...
          <section className="lg:col-span-3 space-y-4 md:space-y-6">
            ...
            {result && (
              <>
                <KpiCards kpis={result.kpis} simStart={result.sim_start} simEnd={result.sim_end} />
                <EquityChart data={result.equity_curve} starting={result.kpis.starting_capital} />
