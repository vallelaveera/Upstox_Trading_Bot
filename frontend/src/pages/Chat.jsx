  ...
  if (isUser) {
    return (
      <div className="flex justify-end">
        <div className="max-w-[75%] bg-[#E2FF00] text-black rounded-2xl rounded-tr-sm px-4 py-3 text-sm font-medium">
          {msg.content}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex gap-3 items-start">
      <div className="h-8 w-8 rounded-lg bg-white/10 flex items-center justify-center shrink-0 mt-0.5">
        <Robot size={16} className="text-[#E2FF00]" />
      </div>
      ...
      <header className="border-b border-white/5 sticky top-0 z-30 backdrop-blur-md bg-[#050505]/80">
        <div className="max-w-[900px] mx-auto px-6 py-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 rounded-lg bg-[#E2FF00] flex items-center justify-center text-black">
              <Robot size={18} weight="bold" />
            </div>
            <div>
              <h1 className="font-display text-lg font-bold tracking-tight leading-none">
                SIGNAL<span className="text-[#E2FF00]">FORGE</span> AI
              </h1>
          ...
          {["", ...KNOWN_STOCKS].map((s) => (
            <button
              ...
              className={`text-xs px-3 py-1 rounded-full border transition-colors ${
                symbol === s
                  ? "bg-[#E2FF00] text-black border-[#E2FF00] font-semibold"
        ...
        <div className="max-w-[900px] mx-auto px-6 py-4">
          <div className="flex gap-3 items-end">
            ...
            <button
              ...
              className="h-12 w-12 rounded-xl bg-[#E2FF00] text-black flex items-center justify-center disabled:opacity-30 hover:bg-[#d4f000] transition-colors shrink-0"
            >
              {loading ? (
